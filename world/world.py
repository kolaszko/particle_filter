...
class World:
    ...
    def show(self, particles, pos_e):
        ...
        # Show landmarks
        self.ax.scatter(self.map.landmarks[:, 0], self.map.landmarks[:, 1], marker='*', color='k')

        # Show particles
        self.ax.scatter(particles[:, 0], particles[:, 1], marker='.', alpha=0.5)

        # Show actual position
        self.ax.scatter(self.agent.x, self.agent.y, marker='o', color='g')

        # Show ground truth path
        p = np.asarray(self.path_gt)
        self.ax.plot(p[:, 0], p[:, 1], color='g')

        # Append estimated pose
        self.path_estimated.append(pos_e)

        # Show estimated pose
        self.ax.scatter(pos_e[0], pos_e[1], marker='o', color='m', alpha=0.5)

        # Show estimated path
        p_hat = np.asarray(self.path_estimated)
        self.ax.plot(p_hat[:, 0], p_hat[:, 1], color='m')
